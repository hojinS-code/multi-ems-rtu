    ...
    try:
        if device.device_type == "single_phase":
            ...
            record = ThreePhaseMeasurement(
                device_id=device.id,
                timestamp=now,
                voltage_r=_decode_float32(vi_regs[0:2]),
                voltage_s=_decode_float32(vi_regs[2:4]),
                voltage_t=_decode_float32(vi_regs[4:6]),
                current_r=_decode_float32(vi_regs[6:8]),
                current_s=_decode_float32(vi_regs[8:10]),
                current_t=_decode_float32(vi_regs[10:12]),
